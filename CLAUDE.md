...
バージョンを重ねながら本番運用に近いインフラを段階的に構築する（v1〜v5 完了、v6 開発中）。
...
### Request Flow
```
Internet → ALB → ECS Fargate (FastAPI + X-Ray SDK) → RDS PostgreSQL
                                                    → S3 (Presigned URL upload)
                                                    → SQS / EventBridge (event publish)
                         ↕ X-Ray Daemon (sidecar, UDP:2000) → X-Ray Service

Internet → CloudFront (OAC) → S3 (attachment download)
Internet → CloudFront (OAC) → S3 (Web UI static assets)

CloudWatch Alarms → SNS Topic → (Email subscription optional)
```

### Event-Driven Flow (v4)
```
Task created  → SQS queue      → Lambda (task_created_handler)  → DLQ on failure
Task completed→ EventBridge bus → Lambda (task_completed_handler)
Scheduler     → EventBridge     → Lambda (task_cleanup_handler)  → RDS direct (VPC内)
```

### Key Design Decisions

- **Graceful degradation**: `SQS_QUEUE_URL`、`EVENTBRIDGE_BUS_NAME`、`S3_BUCKET_NAME`、`CLOUDFRONT_DOMAIN_NAME`、`ENABLE_XRAY` が未設定の場合、対応機能はエラーにならず警告ログのみ出力してスキップ。ローカル開発時に AWS リソース不要。
...
- **`app/main.py`**: エントリポイント。lifespan でマイグレーション実行。`/`, `/health`, routers を include。X-Ray SDK 初期化、CORSMiddleware、構造化ログ（JSONFormatter）を設定（v6）。
...
- **`frontend/`**: React + Vite SPA。S3 + CloudFront でホスティング。タスク CRUD + 添付ファイル操作の管理画面（v6）。
...
**CI** (全 push / PR): `ruff check` → `pytest` (46+ tests) → `docker build` → `npm ci && npm run build` (frontend)
**CD** (main のみ, CI 成功後): ECR push (short-SHA + latest) → ECS rolling deploy → Lambda zip update → Frontend S3 sync + CloudFront invalidation
...
- テスト 46+ 件: test_main.py (基本 API) + test_tasks.py (CRUD + イベント発行) + test_attachments.py (添付ファイル + ファイル名サニタイズ) + test_observability.py (CORS + 構造化ログ + X-Ray graceful degradation)（v6）
