...
```mermaid
graph TD
    subgraph CICD["CI/CD - v9"]
        GH[GitHub - push to main] --> GHA[GitHub Actions - OIDC認証]
        GHA --> CI["CI: Lint → Test → Docker Build → Trivy → tfsec → Terraform Plan → Infracost"]
        GHA --> CD["CD: ECR Push → CodeDeploy B/G Deploy → Lambda → Frontend S3 → Terraform Apply"]
    end
    ...
    subgraph BackendServices["Backend Services"]
        FARGATE --> RDS[RDS PostgreSQL Multi-AZ]
    ...
    subgraph MonitoringGroup["Monitoring - v6"]
        CW_ALARM[CloudWatch Alarms x12] --> SNS["SNS Topic → Email オプション"]
        CW_DASH["CloudWatch Dashboard<br>5行ウィジェット ALB/ECS/RDS/Lambda/SQS"]
    end
```
...
- **CI/CD**: GitHub Actions（OIDC 認証）で自動テスト・CodeDeploy B/G デプロイ・セキュリティスキャン（v9）
...
├── .github/workflows/
│   ├── ci.yml                  # CI: lint, test, build, Trivy, tfsec, terraform plan, Infracost（v9）
│   └── cd.yml                  # CD: OIDC → ECR → CodeDeploy B/G → Lambda → Frontend → terraform apply（v9）
├── tests/
...
```
push / PR
  ├── ci.yml（全ブランチ・PR — OIDC 認証）
  │    ├── lint-test: ruff + pytest (62 テスト)
  │    ├── build: Docker build + Trivy 脆弱性スキャン + SARIF → GitHub Security Tab
  │    ├── security-scan: tfsec Terraform セキュリティスキャン
  │    ├── terraform-plan: plan 実行 → PR コメントに自動投稿
  │    └── infracost: コスト影響 → PR コメント（PR 時のみ）
  │
  └── cd.yml（main のみ、CI 成功後 — workflow_run + OIDC 認証）
...
各バージョンのドキュメントは `_v2`〜`_v9` サフィックスで並行管理している。
