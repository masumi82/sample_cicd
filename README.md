...
graph TD
    ...
    subgraph MonitoringGroup["Monitoring - v6+v10"]
        ...
        CW_DASH["CloudWatch Dashboard<br>9行ウィジェット ALB/ECS/RDS/Lambda/SQS/API GW/Redis/Security"]
    end

    subgraph SecurityMonitoring["Security Monitoring - v13"]
        CT["CloudTrail<br>管理イベント証跡"] --> CT_S3["S3 CloudTrail Logs"]
        CT --> CT_CW["CloudWatch Logs"]
        GD["GuardDuty<br>脅威検出"]
        CONFIG["AWS Config<br>10 マネージドルール"]
        SECHUB["Security Hub<br>CIS + FSBP"]
        GD --> EB_DEF["EventBridge<br>Default Bus"]
        CONFIG --> EB_DEF
        SECHUB --> EB_DEF
        EB_DEF --> SNS
    end

    subgraph DR["DR + Backup - v12"]
        BACKUP["AWS Backup<br>日次+週次"] --> VAULT["Backup Vault"]
        S3_DR_R["S3 DR us-west-2<br>CRR"]
    end
```
...
- **データ保護**: AWS Backup（日次+週次）、RDS Read Replica（読み書き分離）、S3 Versioning + Lifecycle + CRR（v12）
- **セキュリティ監視**: CloudTrail（監査証跡）、GuardDuty（脅威検出）、AWS Config（コンプライアンス 10 ルール）、Security Hub（CIS + FSBP）（v13）
...
データ保護と災害復旧の 4 層防御を追加。

**追加機能**
- AWS Backup（Vault + Plan: 日次 7 日保持 + 週次 30 日保持、タグベース Selection、SNS 通知）
- RDS バックアップ強化（retention 7 日、final_snapshot 有効、deletion_protection 変数化）
- RDS Read Replica（条件付き: dev 無効 / prod 有効、読み取り負荷分散）
- アプリケーション読み書き分離（`get_read_db()` + Graceful degradation フォールバック）
- S3 Versioning 有効化 + Lifecycle Rules（現行 → Standard-IA 90 日、旧 → Glacier 30 日 → 削除 90 日）
- S3 Cross-Region Replication（条件付き: dev 無効 / prod 有効、DR リージョン us-west-2）
- モニタリング拡張（Replica Lag ダッシュボード + アラーム、条件付き）

**学習テーマ**: AWS Backup (Vault/Plan/Selection), RDS Read Replica, 読み書き分離パターン, S3 Versioning, S3 Lifecycle, S3 Cross-Region Replication, Terraform count パターン

---

### v13 — セキュリティ監視 + コンプライアンス（完了）

防御 + 検知 + 監査の 3 層セキュリティを実現。

**追加機能**
- CloudTrail 管理イベント証跡（マルチリージョン、専用 S3 バケット、CloudWatch Logs 連携、ログファイル検証）
- GuardDuty 脅威検出（条件付き、EventBridge → SNS で MEDIUM 以上の findings を通知）
- AWS Config（Recorder + 10 マネージドルール：S3 パブリックアクセス、暗号化、RDS 保護、SSH 制限等）
- Security Hub（CIS Benchmark + AWS Foundational Security Best Practices、Config 依存）
- EventBridge セキュリティイベント通知（default bus → 既存 SNS トピック、3 ルール）
- SNS トピックポリシー（EventBridge + CloudWatch + Backup の publish 許可統合）
- モニタリング拡張（Dashboard Row 9: CloudTrail Events + Config Compliance）
- OIDC IAM 権限追加（cloudtrail, guardduty, config, securityhub）

**学習テーマ**: CloudTrail, GuardDuty, AWS Config (マネージドルール), Security Hub (CIS/FSBP), EventBridge default bus パターン, セキュリティ 3 層モデル（防御 + 検知 + 監査）
...
│   ├── backup.tf               # AWS Backup Vault + Plan + Selection（v12）
│   ├── s3_dr.tf                # DR リージョン S3 + Cross-Region Replication（v12）
│   ├── cloudtrail.tf           # CloudTrail 証跡 + S3 + CloudWatch Logs（v13）
│   ├── guardduty.tf            # GuardDuty Detector + EventBridge→SNS（v13）
│   ├── config.tf               # AWS Config Recorder + 10 Rules + EventBridge→SNS（v13）
│   ├── securityhub.tf          # Security Hub + CIS/FSBP + EventBridge→SNS（v13）
...
│   ├── test_db_routing.py     # v12 読み書き分離テスト
│   └── test_hooks.sh          # v11 Claude Code hook テスト（bash、26 テストケース）
└── docs/
    ├── 01_requirements/        # 要件定義書（v1〜v13）
    ├── 02_design/              # 設計書（アーキテクチャ・API・DB・インフラ・CI/CD）
    ├── 04_test/                # テスト計画書（v1〜v13）
    ├── 05_deploy/              # デプロイ手順書・動作確認記録（v1〜v13）
    ├── 06_learning/            # 学習まとめ（v1〜v13）
    └── guides/                 # ガイド文書
...
各バージョンのドキュメントは `_v2`〜`_v13` サフィックスで並行管理している。
...
| バックアップ | AWS Backup（Vault + Plan + Selection v12） |
| DB レプリカ | RDS Read Replica（読み書き分離 v12） |
| DR | S3 Cross-Region Replication（us-west-2 v12） |
| 監査証跡 | AWS CloudTrail（管理イベント、マルチリージョン v13） |
| 脅威検出 | Amazon GuardDuty（v13） |
| コンプライアンス | AWS Config（10 マネージドルール v13） |
| セキュリティ統合 | AWS Security Hub（CIS + FSBP v13） |
